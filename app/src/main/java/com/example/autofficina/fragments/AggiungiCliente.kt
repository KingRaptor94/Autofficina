...
class AggiungiCliente : Fragment() {
    ...
    private lateinit var mClienteViewModel: ViewModelCliente
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Ottenimento del riferimento al ViewModelCliente
        mClienteViewModel = ViewModelProvider(this).get(ViewModelCliente::class.java)

        // Settaggio del listener per il pulsante di aggiunta cliente
        view.findViewById<Button>(R.id.bottoneAggCliente).setOnClickListener {
            // Recupero dei valori inseriti dall'utente
            val name = view.findViewById<EditText>(R.id.nome_editText).text.toString()
            val surname = view.findViewById<EditText>(R.id.cognome_editText).text.toString()
            val email = view.findViewById<EditText>(R.id.email_editText).text.toString()

            // Aggiunta del nuovo cliente tramite il ViewModelCliente
            mClienteViewModel.salvaClietne(Cliente(0, name, surname, email))
